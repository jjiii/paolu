...
@Component
public class SuccessOrderJob extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext context)
		...
		System.err.println("任务1:successOderJob通知:");
		Map<String, Object> parm = new HashMap<String, Object>();
		parm.put("notify_times", 4);
		parm.put("status", 0);
		parm.put("pay_type", "pay");
		parm.put("limit", 500);
		
		List<NotifyRecord> list = iNotifyRecord.list(parm);
		for(NotifyRecord record : list){
			
			jmsTemplate.send("successOder", new MessageCreator(){
